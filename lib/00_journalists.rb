...
clean = handles.map { |h| h.start_with?('@') ? h[1..-1] : h }

total = handles.size

lengths = clean.map(&:length)
min_len = lengths.min
shortest_handles = handles.select { |h| (h.start_with?('@') ? h[1..-1] : h).length == min_len }


count_len_5 = lengths.count(5)


count_start_upper = clean.count { |c| c[0] =~ /[A-Z]/ }


sorted_alpha = handles.sort_by { |h| (h.start_with?('@') ? h[1..-1] : h).downcase }
